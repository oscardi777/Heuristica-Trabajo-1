...
OUTPUT_FILE   = "resultados\\NWJSSP_OADG_NEH(GRASP).xlsx"

ALPHA                = 0.20   # Voracidad RCL: 0=greedy puro, 1=aleatorio puro
N_ITER               = 10     # Construcciones GRASP por instancia
...
# CONSTRUCCIÓN RCL
# ─────────────────────────────────────────────
def build_rcl(candidates, jobs, alpha):
    """
    Construye la Lista Restringida de Candidatos (RCL) basada en valor.
    Retorna la lista de índices de trabajos que califican para la RCL.
    """
    weights = {
        j: jobs[j].release + sum(op.p for op in jobs[j].operations)
        for j in candidates
    }
    ...
    return [j for j in candidates if weights[j] >= threshold]


# ─────────────────────────────────────────────
# CONSTRUCCIÓN GRASP — una iteración
# ─────────────────────────────────────────────
def construct_grasp_solution(jobs, m, alpha, block_size, time_limit):
    """
    Una iteración de construcción GRASP integrada en el marco NEH:

    En cada paso:
      1. Se calculan los pesos de los trabajos pendientes.
      2. Se construye la RCL basada en valor con parámetro alpha.
      3. Se selecciona aleatoriamente un trabajo j* de la RCL (Uniforme).
      4. Se inserta j* en la mejor posición via búsqueda por bloques
...
# MULTI-START GRASP
# ─────────────────────────────────────────────
def solve(jobs, m, n_iter=N_ITER, alpha=ALPHA, time_limit=TIME_LIMIT_PER_BLOCK):
    """
    Ejecuta n_iter construcciones GRASP y retorna la mejor solución.
    La iteración 0 usa alpha=0 (greedy puro — NEH clásico) como referencia.
    ...
    for i in range(n_iter):
        a = 0.0 if i == 0 else alpha
        seq = construct_grasp_solution(jobs, m, a, block_size, time_limit)
...
def main():
    ...
    for inst in INSTANCES:
        ...
        t0 = time.time()
        sequence, _ = solve(jobs, m, n_iter=N_ITER, alpha=ALPHA)
        ...
        print(
            ...
            f"iters={N_ITER}  alpha={ALPHA}  tiempo={compute_time_ms:>6} ms"
        )
