...
def find_start(job, machines, offsets):
    """
    Encuentra el menor start >= r_j tal que TODAS las operaciones del job
    no se solapen con intervalos ya ocupados en las máquinas.

    Bajo No-Wait:
    op_u ocurre en [start + offset[u], start + offset[u] + p_u]
    """
    start = job.release
    while True:
        feasible = True
        ...
        for u, op in enumerate(job.operations):
            ...
            for b, e in zip(machine.proccess_begin, machine.proccess_end):
                if not (end <= b or begin >= e):
                    feasible = False
                    candidate = e - offsets[u]
        ...
        start = new_start  # garantizado > start por la aritmética del solapamiento


# ─────────────────────────────────────────────
# PROGRAMAR UN TRABAJO
# ─────────────────────────────────────────────
def schedule_job(job, machines, job_id, schedule):
    """
    ...
    for u, op in enumerate(job.operations):
        ...
        finish = begin + op.p
        machine = machines[op.machine]
        machine.proccess_begin.append(begin)
        machine.proccess_end.append(finish)
...
def evaluate_sequence(sequence, jobs, m, save_schedule=False):
    ...
    """
    machines = [Machine(id=i) for i in range(m)]
    total_flow = 0
    schedule = [] if save_schedule else None
    for j in sequence:
        Cj = schedule_job(jobs[j], machines, j, schedule)
        total_flow += Cj
...
# ─────────────────────────────────────────────
# MEJOR POSICIÓN CON BÚSQUEDA POR BLOQUES TEMPORIZADOS + EARLY TERMINATION
# ─────────────────────────────────────────────
def find_best_insertion(sequence, j, jobs, m, block_size, time_limit):
    ...
    búsqueda por bloques con límite de tiempo por bloque y early termination:
      ...
      - Early termination: si hay no_improve_limit posiciones consecutivas
        sin mejora, se interrumpe la búsqueda completamente.
        El límite es adaptativo: max(5, sqrt(k)) donde k es el tamaño
        actual de la secuencia, siendo más permisivo en secuencias grandes
        donde la curva de valores es más irregular.
      - Al finalizar se retorna la mejor posición encontrada.
    """
    n_pos = len(sequence) + 1
    best_pos = 0
    best_value = float("inf")

    # Límite adaptativo: más permisivo para secuencias más largas
    no_improve_limit = max(5, int(math.sqrt(len(sequence) + 1)))
    no_improve_streak = 0
    ...
    while pos < n_pos:
        ...
        for p in range(pos, end_block):
            ...
            if value < best_value:
                best_value = value
                best_pos = p
                no_improve_streak = 0  # hubo mejora → resetear racha
            else:
                no_improve_streak += 1
                if no_improve_streak >= no_improve_limit:
                    return best_pos, best_value  # cortar búsqueda
